...
%         imagesStructRGB(i+1).image(:,:,j) = circshift(imagesStructRGB(i+1).image(:,:,j), [0, -xOffset]);
    end

    % Shift the gray image as well
    imagesStructGray(i+1).image = circshift(imagesStructGray(i+1).image, [-yOffset, 0]);
%     imagesStructGray(i+1).image = circshift(imagesStructGray(i+1).image, [0, -xOffset]);
end

% Iterate through every image and rotate them the images so that they become aligned.
for i = 1:size(imagesStructGray,2)
